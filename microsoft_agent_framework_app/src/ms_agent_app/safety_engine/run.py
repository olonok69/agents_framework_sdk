...
--------
    ...
    python -m ms_agent_app.safety_engine.run --demo

    # Live, all stages, against the Azure/Foundry target (fully wired here):
    python -m ms_agent_app.safety_engine.run \
        --target-provider azure --target-model gpt-4.1 \
        --stages garak,agentdojo,pyrit --out runs/

    # Other providers (garak + AgentDojo wired; PyRIT stage scaffolded):
    python -m ms_agent_app.safety_engine.run --target-provider google --target-model gemini-1.5-pro
    python -m ms_agent_app.safety_engine.run --target-provider bedrock \
        --target-model anthropic.claude-3-5-sonnet-20240620-v1:0

    # Stricter gate for a high-risk important business service:
    python -m ms_agent_app.safety_engine.run --demo --fail-under tool_injection=0.0
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--target-provider", default="demo",
                    help="demo | azure | foundry | google | bedrock | openai")
